...
class PublicDoctorRepository:
    ...
    def list_doctor_rows(self, specialization_id: int | None) -> list[tuple[Doctor, User, int | None, str | None]]:
        stmt = (
            select(
                Doctor,
                User,
            ...
            .outerjoin(DoctorSpecialization, DoctorSpecialization.doctor_user_id == Doctor.doctor_user_id)
            .outerjoin(Specialization, Specialization.specialization_id == DoctorSpecialization.specialization_id)
            .where(Role.role_name == ROLE_DOCTOR)
            .order_by(User.full_name.asc())
        )
        if specialization_id is not None:
            stmt = stmt.where(DoctorSpecialization.specialization_id == specialization_id)
